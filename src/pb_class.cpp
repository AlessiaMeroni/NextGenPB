        ...
        if (num_int_nodes == 0) { //if there's no node inside the molecule
          ...
          // node indices saved here.
          {
            ...
            for (int edge = 0; edge < 12; ++edge) {
              ...
              double frac;
              this->normal_intersection (quadrant, ray_cache, edge, norm, frac);
              pkt_frac[edge] = frac;
              pkt_normals[3*edge + 0] = norm[0];
              pkt_normals[3*edge + 1] = norm[1];
            ...
            bq_local_frac.push_back (pkt_frac);
            bq_local_normals.push_back (pkt_normals);
          }
        }
...
struct EmbeddedPacketPolicy
{
  ...
  {
    for (int inode = 0; inode < 8; ++inode)
      (*pb.border_quad_distributed) (base_idx + inode) =
        (*pb.phi)[pb.bq_local_gnodes[i][inode]];
    for (int inode = 0; inode < 8; ++inode)
      (*pb.border_quad_distributed) (base_idx + 8 + inode) =
  ...
  {
    std::vector<double> & owned = pb.border_quad_distributed->get_owned_data ();
    const int base_idx = PACKET_SIZE * p;
    for (int inode = 0; inode < 8; ++inode) {
      tmp_phi[inode] = owned[base_idx + inode];
      tmp_eps[inode] = owned[base_idx + 8 + inode];
    }
  }
};
...
struct IndexedPacketPolicy
{
  ...
  post_process (poisson_boltzmann & pb)
  {
    ...
    eps_req->get_owned_data () = pb.epsilon_nodes->get_owned_data ();

    std::vector<double> & owned_bq = pb.border_quad_distributed->get_owned_data ();
    ...
    for (int p = 0; p < pb.bq_local_target_count; ++p) {
      ...
      for (int inode = 0; inode < 8; ++inode) {
        const int gnode = static_cast<int> (owned_bq[base_idx + inode]);
        pb.bq_phi_owned[8*p + inode] = (*phi_req)[gnode];
        pb.bq_eps_owned[8*p + inode] = (*eps_req)[gnode];
      }
    }
  }
  ...
  {
    for (int inode = 0; inode < 8; ++inode) {
      tmp_phi[inode] = pb.bq_phi_owned[8*p + inode];
      tmp_eps[inode] = pb.bq_eps_owned[8*p + inode];
    }
  }
};
...
poisson_boltzmann::redistribute_border_quad ()
{
  ...
  for (int i = 0; i < n_local; ++i) {
    ...
    for (int d = 0; d < 3; ++d)
      (*border_quad_distributed) (base_idx + Policy::PREFIX_SIZE + 24 + d) = bq_local_h[i][d];

    for (int e = 0; e < 12; ++e)
      (*border_quad_distributed) (base_idx + Policy::PREFIX_SIZE + 27 + e) = bq_local_frac[i][e];
...
{
  redistribute_border_quad<IndexedPacketPolicy> ();
}
...
poisson_boltzmann::energy_fast (ray_cache_t & ray_cache)
{
  ...
  auto load_packet = [&] (int p) {
    ...
    const int base_idx = Policy::PACKET_SIZE * p;
    for (int c = 0; c < 24; ++c)
      pkt_coords[c] = owned[base_idx + Policy::PREFIX_SIZE + c];
    for (int e = 0; e < 12; ++e)
      pkt_frac[e] = owned[base_idx + Policy::PREFIX_SIZE + 27 + e];
    for (int c = 0; c < 36; ++c)
...
{
  energy_fast<IndexedPacketPolicy> (ray_cache);
}
